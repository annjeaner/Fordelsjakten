...
style.textContent = `
  .breakeven-mobile { display: flex !important; }
  .breakeven-desktop { display: none !important; }
  @media (min-width: 520px) {
    .breakeven-mobile { display: none !important; }
    .breakeven-desktop { display: block !important; }
  }
`;
  ...
  const handleBEmailSubmit = async () => {
    ...
    try {
      await fetch("https://api.convertkit.com/v3/forms/fc5a02df26/subscribe", {
        ...
        body: JSON.stringify({
          api_key: "R_ixnCiQTC2ShSHXzv0RTQ",
          email: bEmail,
          tags: ["variant-b"],
          fields: { monthly_saving: monthlySaving, total_saving: totalSaving }
        })
      });
                ...
                {worthIt ? (
                  <>
                    ...
                    <div style={s.resultStats}>
                      ...
                      {/* Break-even kun på desktop i stats-rad */}
                    </div>

                    {/* Break-even – fremhevet kort */}
                    {breakEven != null && monthlySaving > 0 && (
                      <div style={s.breakEvenCard}>
                        <div style={s.breakEvenCardTop}>
                          <span style={{ fontSize: 28 }}>⏱</span>
                          <div>
                            <div style={{ ...T.label, color: "rgba(255,255,255,0.6)", marginBottom: 4 }}>Break-even punkt</div>
                            <div style={{ fontFamily: "'Agrandir', 'Sora', sans-serif", fontSize: 28, fontWeight: 700, color: "#fff", lineHeight: 1 }}>
...
// ─── COMPONENTS ───────────────────────────────────────────────────────────
// Adaptive month slider med korrekte label-posisjoner
function MonthSlider({ value, onChange }) {
  ...
  // Beregn korrekt prosentposisjon for hvert merke
  ...
  return (
    <div style={{ marginBottom: 14 }}>
      ...
      />
      {/* Labels posisjonert eksakt under riktig punkt på slideren */}
...
const s = {
  ...
  painNumber: { fontFamily: "'Agrandir', 'Sora', sans-serif", fontSize: 42, fontWeight: 700, color: "#fff", textAlign: "center", letterSpacing: -1, marginBottom: 4, textShadow: "0 2px 12px rgba(0,0,0,0.15)" },
  ...
  breakEvenCardDesc: { fontFamily: "'DM Sans', sans-serif", fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.7)", lineHeight: 1.5 },
